...
public class Algorithm {
	private float time;
	private float timeUntilPersonCreate;
	...
	private boolean Update(float deltaTime) {
		...
		if (timeUntilPersonCreate != Constants.INVALID_TIME) {
			timeUntilPersonCreate -= deltaTime;
		}
		for (int i = 0; i < _Elevators.size(); ++i) {
			_Elevators.get(i).Update(time, deltaTime);
		}
		time += deltaTime;
		return true;
	}
	...
	private void CreatePerson() {
		...
		if (_PeopleToCreate == 0) {
			timeUntilPersonCreate = Constants.INVALID_TIME;
		} else {
			timeUntilPersonCreate = Methods.NextFloatExponentialDistribution(_NumberOfFloors);
		}
	}
	...
	// There are two kinds of events: New person creation and Elevator state change
	//The important thing is to always take the least amount of time out of all. If invalid time is returned, that means simulation is over
	private float ProcessTime() {
		float deltaTime = timeUntilPersonCreate;
		for (int i = 0; i < _Elevators.size(); ++i) {
			float elevatorDeltaTime = _Elevators.get(i).DeltaTime(time);
